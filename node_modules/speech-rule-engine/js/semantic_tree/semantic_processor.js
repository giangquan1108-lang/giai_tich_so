...
export class SemanticProcessor {
    ...
    cleanLimitNode(mmlTag, children, length) {
        const isNoSpace = (x) => !x || SemanticPred.isType(x, SemanticType.EMPTY);
        const isSpace = (x) => x &&
    ...
    infixNode_(children, opNode) {
        let node = SemanticProcessor.getInstance().factory_.makeBranchNode(SemanticType.INFIXOP, children, [opNode], SemanticUtil.getEmbellishedInner(opNode).textContent);
    ...
    multiopNode_(operators) {
        const newNode = SemanticProcessor.getInstance().factory_.makeBranchNode(SemanticType.OPERATOR, operators, []);
        newNode.role = SemanticRole.MULTIOP;
    ...
    getTextInRow_(nodes) {
        ...
        const { rel: rel, comp: comp } = SemanticUtil.partitionNodes(nodes, (x) => {
            return (SemanticPred.isType(x, SemanticType.TEXT) && !x.annotation['factor']);
        });
        if (rel.length === 0) {
            return nodes;
        }
