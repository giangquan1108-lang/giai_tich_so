...
const SemanticUtil = __importStar(require("../semantic_tree/semantic_util.js"));
const semantic_util_js_1 = require("../semantic_tree/semantic_util.js");
const EnrichAttr = __importStar(require("./enrich_attr.js"));
...
function enrich(mml, semantic, options) {
    debugger_js_1.Debugger.getInstance().generate(() => [
        'Original MathML',
        formattedOutput(mml)
    ]);
    IDS.clear();
    ...
    if (options.structure) {
        mml.setAttribute(EnrichAttr.Attribute.STRUCTURE, semantic_skeleton_js_1.SemanticSkeleton.fromStructure(mml, semantic, options).toString());
    }
    debugger_js_1.Debugger.getInstance().generate(() => [
        'Semantic Tree\n',
...
function walkTree(semantic) {
    ...
    debugger_js_1.Debugger.getInstance().generate(() => ['WALKING END: ', semantic.toString()]);
...
function insertBefore(node, newChild, oldChild) {
    if (DomUtil.tagName(node) !== semantic_util_js_1.MMLTAGS.MACTION) {
        node.insertBefore(newChild, oldChild);
        return;
    }
    insertBefore(parentNode(node), newChild, node);
}
function isFirstChild(node, child) {
    if (DomUtil.tagName(node) !== semantic_util_js_1.MMLTAGS.MACTION) {
...
function ascendNewNode(newNode, semantic) {
    var _a;
    let empty = semantic && !semantic.hasAnnotation('empty', 'MFENCED')
        && semantic.getAnnotation('empty');
    while (!SemanticUtil.hasMathTag(newNode) &&
        (unitChild(newNode) ||
            (empty && newNode.parentNode && empty.includes((_a = parentNode(newNode).tagName) === null || _a === void 0 ? void 0 : _a.toUpperCase())))) {
