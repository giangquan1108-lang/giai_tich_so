...
const SemanticUtil = __importStar(require("./semantic_util.js"));
...
class SemanticProcessor {
    ...
    static classifyMultiline_(multiline) {
        let index = 0;
    ...
    text(leaf, type) {
        ...
        const meaning = semantic_attr_js_1.SemanticMap.Meaning.get(content[0]);
        if (meaning.type === semantic_meaning_js_1.SemanticType.UNKNOWN ||
            meaning.type === semantic_meaning_js_1.SemanticType.IDENTIFIER) {
            return leaf;
        }
    ...
    limitNode(mmlTag, children) {
        ...
        if (length === 1 && children[1].role === semantic_meaning_js_1.SemanticRole.DEGREE) {
            return SemanticProcessor.getInstance().row([children[0], children[1]]);
        }
    ...
    cleanLimitNode(mmlTag, children, length) {
        const isNoSpace = (x) => !x || SemanticPred.isType(x, semantic_meaning_js_1.SemanticType.EMPTY);
        const isSpace = (x) => x &&
            (SemanticPred.isType(x, semantic_meaning_js_1.SemanticType.TEXT) && SemanticPred.isRole(x, semantic_meaning_js_1.SemanticRole.SPACE));
        const isEmpty = (x) => isNoSpace(x) || isSpace(x);
        if (length === 1) {
        ...
        if (isEmpty(child1)) {
            return [mmlTag === semantic_util_js_1.MMLTAGS.MSUBSUP ? semantic_util_js_1.MMLTAGS.MSUP : semantic_util_js_1.MMLTAGS.MOVER,
                [children[0], child2]];
        }
        if (isEmpty(child2)) {
            return [mmlTag === semantic_util_js_1.MMLTAGS.MSUBSUP ? semantic_util_js_1.MMLTAGS.MSUB : semantic_util_js_1.MMLTAGS.MUNDER,
                [children[0], child1]];
        }
        return [mmlTag, children];
    }
    ...
    infixNode_(children, opNode) {
        let node = SemanticProcessor.getInstance().factory_.makeBranchNode(semantic_meaning_js_1.SemanticType.INFIXOP, children, [opNode], SemanticUtil.getEmbellishedInner(opNode).textContent);
        node.role = opNode.role;
        node = semantic_heuristic_factory_js_1.SemanticHeuristics.run('propagateInterval', node);
    ...
    multiopNode_(operators) {
        const newNode = SemanticProcessor.getInstance().factory_.makeBranchNode(semantic_meaning_js_1.SemanticType.OPERATOR, operators, []);
        newNode.role = semantic_meaning_js_1.SemanticRole.MULTIOP;
    ...
    combineScripts_(nodes) {
        const partition = SemanticUtil.partitionNodes(nodes, (x) => {
            return (SemanticPred.isType(x, semantic_meaning_js_1.SemanticType.SUPERSCRIPT)
                && SemanticPred.isType(x.childNodes[0], semantic_meaning_js_1.SemanticType.EMPTY)) ||
                (SemanticPred.isType(x, semantic_meaning_js_1.SemanticType.SUBSCRIPT)
                    && SemanticPred.isType(x.childNodes[0], semantic_meaning_js_1.SemanticType.EMPTY));
        });
        if (!partition.rel.length) {
            return nodes;
        ...
        do {
            ...
            const last = comp.pop();
            if (SemanticPred.isType(last, semantic_meaning_js_1.SemanticType.NUMBER) ||
                SemanticPred.isType(last, semantic_meaning_js_1.SemanticType.IDENTIFIER)) {
                rel.childNodes[0] = last;
                last.parent = rel;
                rel.role = last.type.toLowerCase();
                rel.addAnnotation('collapsed', last.type.toLowerCase());
                result = result.concat(comp);
                result.push(rel);
                continue;
            }
            if (SemanticPred.isType(last, semantic_meaning_js_1.SemanticType.SUBSCRIPT) &&
                SemanticPred.isType(rel, semantic_meaning_js_1.SemanticType.SUPERSCRIPT)) {
                rel.childNodes[0] = last;
                last.parent = rel;
                rel.role = last.type.toLowerCase();
                rel.addAnnotation('collapsed', last.type.toLowerCase());
                last.role = semantic_meaning_js_1.SemanticRole.SUBSUP;
                result = result.concat(comp);
                result.push(rel);
                continue;
            }
    ...
    getTextInRow_(nodes) {
        ...
        const { rel: rel, comp: comp } = SemanticUtil.partitionNodes(nodes, (x) => {
            return (SemanticPred.isType(x, semantic_meaning_js_1.SemanticType.TEXT) && !x.annotation['factor']);
        });
        if (rel.length === 0) {
            return nodes;
        }
    ...
    horizontalFencedNode_(ofence, cfence, content) {
        ...
        if (SemanticPred.isType(childNode, semantic_meaning_js_1.SemanticType.EMPTY) &&
            !childNode.mathmlTree && ofence.mathmlTree &&
            ofence.mathmlTree.nextSibling !== cfence.mathmlTree) {
            childNode.mathmlTree = ofence.mathmlTree.nextSibling;
            childNode.addMathmlNodes([ofence.mathmlTree.nextSibling]);
        }
    ...
    classifyHorizontalFence_(node) {
        semantic_heuristic_factory_js_1.SemanticHeuristics.run('interval_heuristic', node);
        if (node.role === semantic_meaning_js_1.SemanticRole.INTERVAL) {
            return;
        }
...
function ensureOperatorRelations(before, text, after) {
    const last = before[before.length - 1];
    if (last &&
        (last.type === semantic_meaning_js_1.SemanticType.RELATION || last.type === semantic_meaning_js_1.SemanticType.OPERATOR)) {
        text.addAnnotation('factor', last.type);
        return [...before, text, ...after];
    }
    const first = after[0];
    if (first &&
        (first.type === semantic_meaning_js_1.SemanticType.RELATION ||
            first.type === semantic_meaning_js_1.SemanticType.OPERATOR)) {
        text.addAnnotation('factor', first.type);
        return [...before, text, ...after];
    }
    return null;
}
function annotateFencedNode(node) {
    const meaning1 = semantic_attr_js_1.SemanticMap.FencesSecondary.get(node.contentNodes[0].textContent);
    const meaning2 = semantic_attr_js_1.SemanticMap.FencesSecondary.get(node.contentNodes[1].textContent);
    if (meaning1 === meaning2) {
        node.addAnnotation('fences', meaning1);
    }
}
function isTrivialTable(multiline) {
    return SemanticPred.isType(multiline, semantic_meaning_js_1.SemanticType.MULTILINE) &&
        multiline.childNodes.length === 1 &&
        multiline.childNodes[0].childNodes.length &&
        !SemanticPred.lineIsLabelled(multiline.childNodes[0]);
}
;
function annotateEmpty(tags, node) {
