...
export class SpeechExplorer extends AbstractExplorer {
    ...
    tabKey(event) {
        var _a;
        const active = (_a = this.current) !== null && _a !== void 0 ? _a : (this.node.contains(document.activeElement)
    ...
    tabOut() {
        const html = Array.from(this.node.querySelectorAll('mjx-html'));
        if (html.length) {
            html.forEach((node) => {
    ...
    enterKey(event) {
        if (this.active) {
            if (this.triggerLinkKeyboard(event)) {
                ...
                const tabs = this.getInternalTabs(this.current).filter((node) => !node.getAttribute('data-mjx-href'));
                if (tabs.length) {
                    tabs[0].focus();
                    return;
                }
