...
exports.SvgMo = (function () {
    ...
    return _a = (function (_super) {
            ...
            SvgMo.prototype.stretchSvg = function () {
                ...
                if (this.stretch.dir === FontData_js_2.DIRECTION.Vertical) {
            ...
            SvgMo.prototype.addExtV = function (n, v, H, D, T, B, W) {
                ...
                if (!n)
                    return;
                T = Math.max(0, T - FontData_js_1.VFUZZ);
                B = Math.max(0, B - FontData_js_1.VFUZZ);
            ...
            SvgMo.prototype.addExtH = function (n, v, W, L, R, x) {
                ...
                if (!n)
                    return;
                R = Math.max(0, R - FontData_js_1.HFUZZ);
                L = Math.max(0, L - FontData_js_1.HFUZZ);
                var adaptor = this.adaptor;
                var _b = __read(this.getChar(n, v), 3), h = _b[0], d = _b[1], w = _b[2];
                var X = W - L - R;
                var Y = h + d + 2 * FontData_js_1.VFUZZ;
                var s = 1.5 * (X / w);
                var D = -(d + FontData_js_1.VFUZZ);
                if (X <= 0)
                    return;
                var svg = this.svg('svg', {
                    width: this.fixed(X),
