...
var InfoDialog_js_1 = require("../../ui/dialog/InfoDialog.js");
var isWindows = context_js_1.context.os === 'Windows';
var BRAILLE_PADDING = Array(40).fill('\u2800').join('');
...
var SpeechExplorer = (function (_super) {
    ...
    Object.defineProperty(SpeechExplorer.prototype, "brailleNone", {
        get: function () {
    ...
    SpeechExplorer.prototype.FocusIn = function (event) {
        if (event.target.closest('mjx-html'))
            return;
    ...
    SpeechExplorer.prototype.escapeKey = function (event) {
        if (event.target.closest('mjx-html')) {
            this.refocus = event.target.closest(nav);
            this.Start();
        }
        else {
            this.Stop();
    ...
    SpeechExplorer.prototype.tabKey = function (event) {
        var e_1, _a;
        var _b;
        var active = (_b = this.current) !== null && _b !== void 0 ? _b : (this.node.contains(document.activeElement)
        ...
        var _c = __read(event.shiftKey
            ? [
                this.tabs.slice(0).reverse(),
                Node.DOCUMENT_POSITION_PRECEDING,
                this.current && this.isLink() ? this.getAnchor() : active,
            ]
            : [this.tabs, Node.DOCUMENT_POSITION_FOLLOWING, active], 3), tabs = _c[0], position = _c[1], current = _c[2];
        try {
    ...
    SpeechExplorer.prototype.tabTo = function (node) {
    ...
    SpeechExplorer.prototype.tabOut = function () {
        var html = Array.from(this.node.querySelectorAll('mjx-html'));
        if (html.length) {
    ...
    SpeechExplorer.prototype.enterKey = function (event) {
        if (this.active) {
            if (this.triggerLinkKeyboard(event)) {
                ...
                var tabs = this.getInternalTabs(this.current).filter(function (node) { return !node.getAttribute('data-mjx-href'); });
                if (tabs.length) {
                    tabs[0].focus();
                    return;
                }
    ...
    SpeechExplorer.prototype.toggleBraille = function () {
        var value = !this.document.options.a11y.brailleCombine;
        if (this.document.menu) {
    ...
    SpeechExplorer.prototype.help = function () {
        ...
        var CLASS = this.constructor;
        var _a = __read(CLASS.helpData.get(context_js_1.context.os), 3), title = _a[0], select = _a[1], braille = _a[2];
        InfoDialog_js_1.InfoDialog.post({
    ...
    SpeechExplorer.prototype.setCurrent = function (node, addDescription) {
        ...
        if (this.current) {
            ...
            try {
                for (var _c = __values(Array.from(this.node.querySelectorAll('.mjx-selected'))), _d = _c.next(); !_d.done; _d = _c.next()) {
                    var part = _d.value;
        ...
        this.currentMark = -1;
        if (this.current) {
            var parts = __spreadArray([], __read(this.getSplitNodes(this.current)), false);
    ...
    SpeechExplorer.prototype.getSplitNodes = function (node) {
        ...
        var parts = Array.from(this.node.querySelectorAll("[data-semantic-id=\"".concat(id, "\"]")));
        var subtree = this.subtree(id, parts);
        this.cacheParts.set(id, __spreadArray(__spreadArray([], __read(parts), false), __read(subtree), false));
        return this.cacheParts.get(id);
    };
    SpeechExplorer.prototype.subtree = function (id, nodes) {
        var e_4, _a;
        var _this = this;
        var sub = this.subtrees.get(id);
        var children = new Set();
        try {
            for (var nodes_1 = __values(nodes), nodes_1_1 = nodes_1.next(); !nodes_1_1.done; nodes_1_1 = nodes_1.next()) {
                var node = nodes_1_1.value;
                Array.from(node.querySelectorAll("[data-semantic-id]")).forEach(function (x) { return children.add(_this.nodeId(x)); });
            }
        }
        ...
        var rest = setdifference(sub, children);
        return __spreadArray([], __read(rest), false).map(function (child) { return _this.getNode(child); })
    ...
    SpeechExplorer.prototype.addSpeech = function (node, describe) {
        ...
        if (this.anchors.length) {
            setTimeout(function () { var _a; return (_a = _this.img) === null || _a === void 0 ? void 0 : _a.remove(); }, 10);
        }
        else {
            (_a = this.img) === null || _a === void 0 ? void 0 : _a.remove();
        }
        var speech = this.addComma([
    ...
    SpeechExplorer.prototype.addComma = function (words) {
    ...
    SpeechExplorer.prototype.speak = function (speech, braille, ssml, description) {
        ...
        var speechNode = (this.speech = document.createElement('mjx-speech'));
        ...
        speechNode.setAttribute(SpeechUtil_js_1.SemAttr.SPEECH, speech);
        if (ssml) {
            speechNode.setAttribute(SpeechUtil_js_1.SemAttr.PREFIX_SSML, ssml[0] || '');
            speechNode.setAttribute(SpeechUtil_js_1.SemAttr.SPEECH_SSML, ssml[1] || '');
            speechNode.setAttribute(SpeechUtil_js_1.SemAttr.POSTFIX_SSML, ssml[2] || '');
        }
        if (braille) {
        ...
        if (isWindows) {
            var container = document.createElement('mjx-speech-container');
        ...
        this.Update();
        if (oldspeech) {
            setTimeout(function () { return _this.unspeak(oldspeech); }, 100);
        }
    };
    SpeechExplorer.prototype.unspeak = function (node) {
        if (isWindows) {
    ...
    SpeechExplorer.prototype.attachSpeech = function () {
        ...
        var braille = container.getAttribute(SpeechUtil_js_1.SemAttr.BRAILLE);
        if (braille) {
            if (this.document.options.a11y.brailleSpeech) {
    ...
    SpeechExplorer.prototype.adjustAnchors = function () {
        var e_7, _a;
        this.anchors = Array.from(this.node.querySelectorAll('a[href]'));
        try {
            for (var _b = __values(this.anchors), _c = _b.next(); !_c.done; _c = _b.next()) {
                var anchor = _c.value;
    ...
    SpeechExplorer.prototype.restoreAnchors = function () {
        var e_8, _a;
        try {
            for (var _b = __values(this.anchors), _c = _b.next(); !_c.done; _c = _b.next()) {
                var anchor = _c.value;
    ...
    SpeechExplorer.prototype.getTabs = function () {
        this.tabs = this.getInternalTabs(this.node);
    };
    SpeechExplorer.prototype.getInternalTabs = function (node) {
        return Array.from(node.querySelectorAll('button, [data-mjx-href], input, select, textarea, [tabindex]:not([tabindex="-1"],mjx-speech)'));
    };
    ...
    SpeechExplorer.prototype.findClicked = function (node, x, y) {
        ...
        while (clicked) {
            ...
            clicked = null;
            try {
                for (var nodes_2 = (e_10 = void 0, __values(nodes)), nodes_2_1 = nodes_2.next(); !nodes_2_1.done; nodes_2_1 = nodes_2.next()) {
                    var child = nodes_2_1.value;
    ...
    SpeechExplorer.prototype.isLink = function (node) {
        var _a;
        if (node === void 0) { node = this.current; }
        return !!((_a = node === null || node === void 0 ? void 0 : node.getAttribute('data-semantic-attributes')) === null || _a === void 0 ? void 0 : _a.includes('href:'));
    };
    SpeechExplorer.prototype.getAnchor = function (node) {
        if (node === void 0) { node = this.current; }
        var anchor = node.closest('a');
        return anchor && this.node.contains(anchor) ? anchor : null;
    };
    SpeechExplorer.prototype.linkFor = function (anchor) {
        return anchor === null || anchor === void 0 ? void 0 : anchor.querySelector('[data-semantic-attributes*="href:"]');
    };
    SpeechExplorer.prototype.parentLink = function (node) {
        var link = node === null || node === void 0 ? void 0 : node.closest('[data-semantic-attributes*="href:"]');
    ...
    Object.defineProperty(SpeechExplorer.prototype, "tabIndex", {
        get: function () {
    ...
    SpeechExplorer.prototype.addHtmlEvents = function () {
        var e_11, _a;
        var _this = this;
        var _loop_1 = function (html) {
            var stop_1 = function (event) {
                if (html.contains(document.activeElement)) {
                    if (event instanceof KeyboardEvent) {
                        ...
                        _this.clicked = event.target;
                    }
                }
            };
            html.addEventListener('mousedown', stop_1);
            html.addEventListener('click', stop_1);
            html.addEventListener('keydown', stop_1);
            html.addEventListener('dblclick', stop_1);
        };
        try {
            for (var _b = __values(Array.from(this.node.querySelectorAll('mjx-html'))), _c = _b.next(); !_c.done; _c = _b.next()) {
                var html = _c.value;
                _loop_1(html);
            }
        }
    ...
    SpeechExplorer.prototype.triggerLink = function (node) {
        var _this = this;
        if (this.isLink(node)) {
            var anchor = this.getAnchor(node);
    ...
    SpeechExplorer.prototype.triggerLinkMouse = function () {
        var link = this.parentLink(this.refocus);
    ...
    SpeechExplorer.prototype.getSubtrees = function () {
        var node = this.node.querySelector('[data-semantic-structure]');
        if (!node)
            return;
        var sexp = node.getAttribute('data-semantic-structure');
    ...
    SpeechExplorer.keyMap = new Map(__spreadArray([
        ['Tab', [function (explorer, event) { return explorer.tabKey(event); }]],
        ['Escape', [function (explorer, event) { return explorer.escapeKey(event); }]],
        ...
        ['b', [function (explorer) { return explorer.toggleBraille(); }, false]]
...
function buildMap(tree, map) {
    ...
    var _b = __read(tree), root = _b[0], children = _b.slice(1);
    var rootId = root;
    var descendants = new Set();
    try {
        for (var children_1 = __values(children), children_1_1 = children_1.next(); !children_1_1.done; children_1_1 = children_1.next()) {
            var child = children_1_1.value;
            var childRoot = typeof child === 'string' ? child : child[0];
            var childDescendants = buildMap(child, map);
            descendants.add(childRoot);
            childDescendants.forEach(function (d) { return descendants.add(d); });
        }
    }
...
function setdifference(a, b) {
    ...
    return new Set(__spreadArray([], __read(a), false).filter(function (x) { return !b.has(x); }));
}
