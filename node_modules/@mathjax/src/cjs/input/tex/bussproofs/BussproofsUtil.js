...
var _previousSibling = function (inf) {
    return inf.parent.childNodes[inf.parent.childNodes.indexOf(inf) - 1];
};
...
var getSpaces = function (inf, rule, right) {
    ...
    if (inf !== rule.parent) {
        result +=
            inf.getProperty(right ? 'proof-right' : 'proof-left') || 0;
        inf = rule.parent;
    }
    if (inf === rule) {
        return result;
...
var addSpace = function (config, inf, space, right) {
    ...
    var prop = right ? 'proof-right' : 'proof-left';
    inf.setProperty(prop, (inf.getProperty(prop) || 0) + space);
    var styles = [];
    try {
        for (var _b = __values(['left', 'right']), _c = _b.next(); !_c.done; _c = _b.next()) {
            var side = _c.value;
            var margin = inf.getProperty("proof-".concat(side));
            if (margin) {
                styles.push("margin-".concat(side, ": ").concat(UnitUtil_js_1.UnitUtil.em(margin)));
            }
        }
    }
...
var moveProperties = function (src, dest) {
    var props = ['inference', 'proof', 'labelledRule'];
...
var addSequentSpace = function (config, sequent, position, direction, width) {
    var mspace = config.nodeFactory.create('node', 'mspace', [], {
        width: UnitUtil_js_1.UnitUtil.em(width),
    });
    if (direction === 'left') {
...
var balanceRules = function (arg) {
    ...
    try {
        for (var inferences_1 = __values(inferences), inferences_1_1 = inferences_1.next(); !inferences_1_1.done; inferences_1_1 = inferences_1.next()) {
            ...
            var delta = (getBBox(rule) - getBBox(premises.parent)) / 2;
            ...
            if (isProof || !(column = getColumn(inf))) {
                addSpace(config, (0, exports.getProperty)(inf, 'proof') ? inf : inf.parent, maxAdjust, true);
                continue;
            }
            var sibling = nextSibling(column);
            if (sibling) {
                var pos = config.nodeFactory.create('node', 'mspace', [], {
                    width: UnitUtil_js_1.UnitUtil.em(maxAdjust),
                });
...
var prefix_pattern = RegExp('^' + property_prefix);
