  ...
  const renderSolutionMatrix = (sol: number[][]) => {
    ...
    return <Box>
      ...
      <Box sx={{ fontFamily: 'monospace', fontSize: '0.95rem', overflowX: 'auto' }}>
        {sol.map((row, ri) => <Box key={ri}>| {row.map((v: number) => v.toFixed(6).padStart(12)).join(' ')} |</Box>)}
      </Box>
    </Box>;
  };
    ...
    <Container maxWidth="lg">
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Grid container spacing={3}>
          ...
          {showIterativeParams && (<>
            ...
            {method === 'sor' && <Grid size={{ xs: 6, md: 2 }}><TextField fullWidth type="text" inputMode="decimal" label="Omega (ω)" value={omega} onChange={(e) => setOmega(e.target.value)} size="small" /></Grid>}
            <Grid size={{ xs: 12, md: method === 'sor' ? 2 : 4 }}><TextField fullWidth type="text" label="Initial guess (cách nhau bởi dấu ,)" value={initialGuessStr} onChange={(e) => setInitialGuessStr(e.target.value)} helperText={`VD: ${Array(aCols).fill('0').join(', ')}`} size="small" /></Grid>
          </>)}
      ...
      {result && (<>
        {result.solution_type === 'unique' && result.solution && result.solution.length > 0 && <ResultCard success={result.success} message={result.message}>
          <Box sx={{ mt: 1 }}>{renderSolutionMatrix(result.solution)}{result.execution_time && <Typography variant="body2" color="text.secondary">⏱ {result.execution_time.toFixed(6)}s</Typography>}</Box>
        </ResultCard>}
        ...
        {result.solution_type === 'infinite' && <ResultCard success={true} message={result.message}>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {result.particular_solution && result.particular_solution.length > 0 && <Paper variant="outlined" sx={{ p: 2 }}><Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 1 }}>📌 Nghiệm riêng x<sub>p</sub></Typography><Box sx={{ fontFamily: 'monospace', fontSize: '0.9rem', overflowX: 'auto' }}>{(result.particular_solution[0] as number[]).map((_, ci) => <Box key={ci} sx={{ mb: 1 }}><Typography variant="body2" sx={{ fontWeight: 600 }}>Cột {ci + 1}: [{(result.particular_solution as number[][]).map(row => row[ci].toFixed(6)).join(', ')}]<sup>T</sup></Typography></Box>)}</Box></Paper>}
            {result.basis_vectors && result.basis_vectors.length > 0 && <Paper variant="outlined" sx={{ p: 2 }}><Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 1 }}>🧱 Vector cơ sở</Typography>{result.basis_vectors.map((bv, idx) => <Box key={idx} sx={{ mb: 1.5 }}><Typography variant="body2" sx={{ fontWeight: 600 }}>v<sub>{idx + 1}</sub> = [{bv.map(v => v.toFixed(6)).join(', ')}]<sup>T</sup></Typography><Box sx={{ fontFamily: 'monospace', fontSize: '0.9rem', borderLeft: '2px solid', borderRight: '2px solid', borderColor: 'primary.main', display: 'inline-block', px: 1, py: 0.5, lineHeight: 1.5 }}>{bv.map((v, ri) => <Box key={ri}>{v.toFixed(6)}</Box>)}</Box></Box>)}</Paper>}
        ...
        {isDirect(method) && result.steps && result.steps.length > 0 && <SolutionSteps steps={result.steps as any} method={method} />}
      </>)}
    </Container>
  );
}